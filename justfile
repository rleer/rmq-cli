...
prepare-e2e-test:
    rm -rf test/rmq.E2E.Tests/bin/rmq-published
    dotnet publish src/rmq/rmq.csproj -c Release -r osx-arm64 -o test/rmq.E2E.Tests/bin/rmq-published
...
run *args:
    @dotnet run --project src/rmq/rmq.csproj --no-build --no-launch-profile -- {{args}}

# Create alias for running rmq via dotnet run (use: eval $(just create-alias))
create-alias:
    @echo "alias rmq-dev='dotnet run --project {{justfile_directory()}}/src/rmq/rmq.csproj --no-build --no-launch-profile --'"
